...
public class ProductMapHandler extends BotCommandHandler {
    ...
    public BotApiMethod<?> handle(Message message, ChatSession session) {
        ...
        return switch (step) {
            ...
            case UPDATE_LIST -> updateList(session, locale, message.getMessageId());
            ...
            case DELETE_LIST -> deleteList(session, locale, message.getMessageId());
    ...
    private EditMessageText updateList(ChatSession session, Locale locale, int messageId) {
        String text = messageSource.getMessage(Steps.UPDATE_LIST.name(), null, locale);
    ...
    private EditMessageText selectToUpdate(ChatSession session, Locale locale, String messageText, int messageId) {
        ...
        session.addAttribute(Attributes.PRODUCT_TYPE_TO_UPDATE.name(), productTypeId.toString());
        String text = messageSource.getMessage(Steps.SELECT_TO_UPDATE.name(), new Object[]{productType.getTitle()}, locale);
    ...
    private SendMessage updateInput(ChatSession session, Locale locale, String messageText) {
        ...
        String productMapId = session.getAttribute(Attributes.PRODUCT_TYPE_TO_UPDATE.name());
        if (productMapId == null || productMapId.isBlank()) {
            throw new IllegalArgumentException("ChatSession attribute '%s' is null or empty".formatted(Attributes.PRODUCT_TYPE_TO_UPDATE));
        }
        UUID id = UUID.fromString(productMapId);
        ProductMap productMap = productMapService.updatePrice(id, newPrice, session.getUserId());
        return sendProductMapAsMessage(productMap, session, locale);
    }

    private EditMessageText deleteList(ChatSession session, Locale locale, int messageId) {
        String text = messageSource.getMessage(Steps.DELETE_LIST.name(), null, locale);
    ...
    private SendMessage sendProductMapAsMessage(ProductMap productMap, ChatSession session, Locale locale) {
        ...
        String callbackQueryPrefix = ChatBotUtility.callBackQueryPrefix(BotCommands.PRODUCT_MAP, Steps.UPDATE_LIST.ordinal());
        InlineKeyboardButton update = keyboardBuilderKit.callbackButton(ButtonKeys.UPDATE, callbackQueryPrefix, locale);
        callbackQueryPrefix = ChatBotUtility.callBackQueryPrefix(BotCommands.PRODUCT_MAP, Steps.DELETE_LIST.ordinal());
    ...
    private InlineKeyboardMarkup productMapAsCallbackQuery(ChatSession session, Locale locale, Steps step) {
        ...
        InlineKeyboardButton backButton = keyboardBuilderKit.callbackButton(ButtonKeys.BACK, callbackQueryPrefix, locale);
